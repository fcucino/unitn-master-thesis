...
About the tool, the different images are needed for the devices which run the \textit{arm} architectures. \\
...
The automatic build and deploy is managed by the Google Cloud Build service. It is integrated with the repositories of the projects and it is triggered by the push of the code to the repository on a specific tag or on the master branch. The service is configured by a configuration file which contains all the steps that are needed to build the images. The images are then pushed to a remote registry and then they are automatically deployed to the cluster. Instead, the built tool binaries are stored in a public storage bucket. The bucket always contains the reference to the latest master version and all the previous tagged versions. We decided to store the binaries in a public storage bucket because of the simplicity of the service for our intership project.
...
Notice that, altough the app is accessible using a private account but the download link is public, we do not consider this a security issue because of the initial stage of the project and the limited access to the preview version of the app. In fact, only the company testers can access it at this time. Future work will be to implement a proper authentication system to access the download link.
